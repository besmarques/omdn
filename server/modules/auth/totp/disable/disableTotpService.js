...
export default function createDisableTotpService(authRepository) {
	...
	async function verifyRecoveryCode({ userId, code, connection }) {
		const codeHash = hashRecoveryCode(code);

		const recoveryCode = await authRepository.findUnusedRecoveryCodeForUpdate(userId, codeHash, connection);

		return Boolean(recoveryCode);
	}

	return async function disableTotp({ userId, password, code, currentSessionId }) {
		if (!currentSessionId) {
		...
		return authRepository.withConnection(async (connection) => {
			try {
				...
				const authenticationIsValid = isTotpCode(code)
				...
				await authRepository.deleteRecoveryCodes(userId, connection);

				const deletedTotpRows = await authRepository.deleteTotp(userId, connection);

				if (deletedTotpRows !== 1) {
					throw new Error('Unable to disable TOTP');
				}
