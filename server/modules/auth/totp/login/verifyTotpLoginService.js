...
export default function createVerifyTotpLoginService(dependencies, decryptSecret = decryptTotpSecret) {
	...
	async function verifyTotpCode({ userId, code, totp, connection }) {
		...
		const affectedRows = await authRepository.updateTotpLastUsedStep(userId, verification.timeStep, connection);
	...
	async function verifyRecoveryCode({ userId, code, connection }) {
		...
		const affectedRows = await authRepository.markRecoveryCodeUsed(recoveryCode.id, connection);
	...
	async function verifySecondFactor({ userId, code }) {
		return authRepository.withConnection(async (connection) => {
			try {
				...
				const method = isTotpCode(code) ? 'totp' : 'recovery_code';
				...
				const user = await authRepository.findActiveUserById(userId, connection);
