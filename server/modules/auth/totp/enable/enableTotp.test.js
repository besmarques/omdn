...
vi.mock(
	'#server/modules/auth/totp/shared/recoveryCodes',
	() => ({
...
function createRepositoryMock({
	...
	const authRepository = {
		...
		findTotpByUserIdForUpdate: vi
			.fn()
			.mockResolvedValue(totp),

		replaceRecoveryCodes: vi
			.fn()
			.mockResolvedValue(),

		enableTotp: vi
			.fn()
			.mockResolvedValue(enabledRows),
	};
...
describe('enable TOTP', () => {
	beforeEach(() => {
		...
		decryptTotpSecret.mockReturnValue(
			'BASE32SECRET',
		);

		generateRecoveryCodes.mockReturnValue(
			recoveryCodes,
		);

		hashRecoveryCode.mockImplementation(
			(code) => `hash:${code}`,
		);
	});
	...
	it('enables a pending TOTP configuration', async () => {
		...
		expect(hashRecoveryCode).toHaveBeenCalledTimes(
			recoveryCodes.length,
		);

		expect(
			authRepository.replaceRecoveryCodes,
		).toHaveBeenCalledWith(
			42,
			[
				'hash:AAAA-BBBB-CCCC-DDDD-EEEE',
				'hash:FFFF-GGGG-HHHH-IIII-JJJJ',
			],
			connection,
		);

		expect(
			authRepository.enableTotp,
		).toHaveBeenCalledWith(
			42,
			101,
			connection,
